...
function par_list($i, $par, $stats, &$totaltoday, &$totaltotal, $proj_scale, $color_a = "", $color_b = "")
{
    ...
    ?>
    <tr>
      <td><?echo $stats->get_stats_item("overall_rank") . html_rank_arrow($stats->get_stats_item("overall_change")) ?></td>
      <td><a href="psummary.php?project_id=<?=$gproj->get_id()?>&amp;id=<?=$parid?>"><?=safe_display($participant)?></a></td>
      <td><?echo number_style_convert($stats->get_stats_item("days_working"));?></td>
      <td><?echo number_style_convert($stats->get_stats_item("work_total") * $proj_scale) ?> </td>
      <td><?echo number_style_convert($stats->get_stats_item("work_today") * $proj_scale) ?> </td>
    </tr>
    <?
}
...
function par_footer($totaltoday, $totaltotal, $proj_scale)
{
    ...
    <tfoot>
      ...
      <td><?echo number_style_convert($totaltotal * $proj_scale)?></td>
      <td><?echo number_style_convert($totaltoday * $proj_scale)?></td>
    </tfoot>
    <?
}
...
?>
  <div class="panel panel-default">
    <div class="panel-heading">
      <?=safe_display($gpart->get_display_name())?>'s stats
    </div>
    <?php if($gpart -> get_motto() <> '') { ?> 
    <div class="panel-body">
      <i><?php echo markup_to_html($gpart->get_motto());?></i>
    </div>
    <?php } ?>
  </div>

 <?php if( $gpartstats->are_stats_loaded() ) { ?>
...
<?php } else { // stats loaded end ?>
      <div class="well">This Participant hasn't participated in this project</div>
<?php } ?>
